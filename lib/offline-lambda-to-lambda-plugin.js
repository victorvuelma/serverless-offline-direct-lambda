...
class OfflineLambdaToLambdaPlugin {
  ...
  proxyFunction (serverlessFunction, location) {
    ...
    const awsSDKNodeEvent = this.createNodeSdkEvent(serverlessFunction, location)
    if (awsSDKNodeEvent) events.push(awsSDKNodeEvent)

    const awsSDKEvent = this.createSdkEvent(serverlessFunction, location)
    if (awsSDKEvent) events.push(awsSDKEvent)
  ...
  createNodeSdkEvent (serverlessFunction, location) {
  ...
  createSdkEvent (serverlessFunction, location) {
    return this.createEvent(
      serverlessFunction,
